...
function AuthScreen({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isRegister) { setIsRegister(false); setError('Account created! Please log in.'); } 
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-blue-100 p-4">
      <div className="glass-panel p-10 rounded-3xl shadow-2xl w-full max-w-md">
        <div className="text-center mb-10">
          <div className="bg-indigo-600 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg shadow-indigo-300 transform rotate-3"><Clock className="text-white" size={32} /></div>
          <h1 className="text-3xl font-extrabold text-slate-800">{isRegister ? 'Join TimeApp' : 'Welcome Back'}</h1>
        </div>
...
function Sidebar({ user, activeView, onChangeView, onLogout, isTimerRunning }) {
  ...
  return (
    <aside className="w-72 bg-slate-900 text-slate-300 flex flex-col hidden md:flex h-full shadow-2xl z-20 shrink-0">
      ...
      {isTimerRunning && <div className="mx-6 mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center gap-3 animate-pulse"><div className="w-2.5 h-2.5 bg-red-500 rounded-full"></div><span className="text-xs font-bold text-red-400 uppercase tracking-wide">Tracking Time</span></div>}
...
function MainContent({ user, view, onTimerChange }) {
  ...
  return (
    <main className="flex-1 p-8 md:p-12 overflow-y-auto bg-slate-100/50">
      ...
      {view === 'tracker' && <div className="max-w-6xl space-y-8"><TimeTrackerCard onUpdate={update} onTimerChange={onTimerChange}/><div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div className="lg:col-span-2"><HistoryList trigger={trigger} onUpdate={update}/></div><div><ManualEntryCard onUpdate={update}/></div></div></div>}
...
function AnalyticsView({ trigger }) {
  ...
  const projectData = useMemo(() => {
    ...
    data.forEach(item => { const pName = item.project_name || 'Unknown'; if (!map[pName]) map[pName] = { name: pName }; map[pName][item.user_name] = Number(item.hours); usersSet.add(item.user_name); });
  ...
  return (
    <div className="space-y-10 max-w-7xl pb-20">
      <div className="glass-panel p-8 rounded-3xl">
        ...
        <div className="h-[450px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart layout="vertical" data={projectData.chartData} margin={{right: 30}} barSize={30}><CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={true} stroke="#e2e8f0"/><XAxis type="number" stroke="#94a3b8"/><YAxis dataKey="name" type="category" stroke="#64748b" width={100}/><Tooltip cursor={{fill: '#f1f5f9'}}/><Legend wrapperStyle={{paddingTop: '20px'}}/>{projectData.users.map((userName, i) => (<Bar key={userName} dataKey={userName} stackId="a" fill={COLORS[i % COLORS.length]} radius={[0, 4, 4, 0]}><LabelList dataKey={userName} position="center" style={{ fill: '#fff', fontSize: '11px', fontWeight: 'bold' }} formatter={(val) => val > 0.5 ? val : ''} /></Bar>))}</BarChart></ResponsiveContainer></div>
      </div>
...
// --- TRACKER & HISTORY (With EDIT/DELETE) ---
...
function HistoryList({ trigger, onUpdate }) {
  ...
  // Load projects for the edit dropdown
  ...
  const handleSaveEdit = async (e) => {
    ...
    await authFetch(`/entries/${editingEntry.id}`, {
      method: 'PUT',
      body: JSON.stringify({ projectId: editingEntry.project_id, start, end })
    });
        ...
        <div className="overflow-y-auto flex-1 p-2">
          {entries.map(e=><div key={e.id} className="p-4 mb-2 rounded-2xl flex justify-between items-center hover:bg-white hover:shadow-md transition-all group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-md" style={{backgroundColor: e.color || '#cbd5e1'}}><Clock size={18}/></div>
              <div><div className="font-bold text-slate-700">{e.project_name}</div><div className="text-xs text-slate-400 font-medium">{new Date(e.start_time).toLocaleDateString()} • {new Date(e.start_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div></div>
            </div>
            <div className="flex items-center gap-3">
              <span className="font-mono text-sm font-bold bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg border border-slate-200">{Math.round(e.duration_seconds/60)}m</span>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => openEdit(e)} className="p-2 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-lg"><Edit2 size={16}/></button>
                <button onClick={() => handleDelete(e.id)} className="p-2 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-lg"><Trash2 size={16}/></button>
              </div>
            </div>
          </div>)}
      ...
      <Modal isOpen={!!editingEntry} onClose={() => setEditingEntry(null)} title="Edit Entry">
        {editingEntry && (
          <form onSubmit={handleSaveEdit} className="space-y-4">
            <div className="space-y-1"><label className="text-xs font-bold text-slate-400 uppercase">Project</label>
            <select className="glass-input w-full p-3 rounded-xl" value={editingEntry.project_id} onChange={e => setEditingEntry({...editingEntry, project_id: e.target.value})}>
              {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select></div>
            <div className="space-y-1"><label className="text-xs font-bold text-slate-400 uppercase">Date</label>
            <input type="date" className="glass-input w-full p-3 rounded-xl" value={editingEntry.date} onChange={e => setEditingEntry({...editingEntry, date: e.target.value})}/></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><label className="text-xs font-bold text-slate-400 uppercase">Start</label><input type="time" className="glass-input w-full p-3 rounded-xl" value={editingEntry.startTime} onChange={e => setEditingEntry({...editingEntry, startTime: e.target.value})}/></div>
              <div className="space-y-1"><label className="text-xs font-bold text-slate-400 uppercase">End</label><input type="time" className="glass-input w-full p-3 rounded-xl" value={editingEntry.endTime} onChange={e => setEditingEntry({...editingEntry, endTime: e.target.value})}/></div>
            </div>
...
// --- PROJECT MANAGER (With EDIT/DELETE) ---
...
function ProjectsManager() {
  ...
  const handleSaveEdit = async (e) => {
    e.preventDefault();
    await authFetch(`/projects/${editingProject.id}`, { method: 'PUT', body: JSON.stringify({ name: editingProject.name, color: editingProject.color }) });
    ...
    <>
      <div className="glass-panel p-8 rounded-3xl max-w-4xl">
        ...
        <form onSubmit={add} className="flex gap-4 mb-8">
          <input value={name} onChange={e=>setName(e.target.value)} className="glass-input p-4 rounded-xl w-full font-bold outline-none" placeholder="New project name..." />
          <button className="bg-indigo-600 text-white px-8 rounded-xl font-bold shadow-lg shadow-indigo-200">Create</button>
        </form>
        <div className="grid gap-3">
          {projects.map(p=><div key={p.id} className="p-4 rounded-xl bg-white border border-slate-100 flex justify-between items-center group hover:shadow-md transition-all">
            <span className="font-bold text-slate-700">{p.name}</span>
            <div className="flex gap-2">
              <button onClick={() => setEditingProject(p)} className="p-2 text-slate-300 hover:text-indigo-600 transition-colors"><Edit2 size={18}/></button>
              <button onClick={() => del(p.id)} className="p-2 text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={18}/></button>
            </div>
          </div>)}
        </div>
      </div>
      ...
      <Modal isOpen={!!editingProject} onClose={() => setEditingProject(null)} title="Edit Project">
        {editingProject && (
          <form onSubmit={handleSaveEdit} className="space-y-4">
             <div className="space-y-1"><label className="text-xs font-bold text-slate-400 uppercase">Project Name</label>
             <input className="glass-input w-full p-3 rounded-xl" value={editingProject.name} onChange={e => setEditingProject({...editingProject, name: e.target.value})}/></div>
...
function TimeTrackerCard({ onUpdate, onTimerChange }) {
  const [projects, setProjects] = useState([]); const [timer, setTimer] = useState(null); const [proj, setProj] = useState('');
  useEffect(() => { authFetch('/projects').then(r=>r.json()).then(setProjects) }, []);
  ...
  const toggle = async () => { if (timer) { await authFetch('/entries/stop', { method: 'POST' }); setTimer(null); onUpdate(); onTimerChange(false); } else { if(!proj) return alert('Select project'); const res = await authFetch('/entries/start', { method: 'POST', body: JSON.stringify({ projectId: proj }) }); if(res.ok) { setTimer(await res.json()); onTimerChange(true); } } };
  return (
    <div className="glass-panel p-8 rounded-3xl flex flex-col md:flex-row items-end gap-6 relative overflow-hidden">
      {timer && <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-red-500 via-orange-500 to-red-500 animate-pulse"></div>}
      <div className="flex-1 w-full space-y-2"><label className="text-xs font-extrabold text-slate-400 uppercase tracking-widest ml-1">Current Task Project</label>
      <div className="relative"><FolderKanban className="absolute left-4 top-4 text-slate-400" size={20}/><select className="glass-input w-full pl-12 p-4 rounded-xl text-slate-700 font-bold outline-none appearance-none" value={proj} onChange={e=>setProj(e.target.value)} disabled={!!timer}><option value="">Select a Project...</option>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div></div>
      <button onClick={toggle} className={`w-full md:w-auto p-4 px-10 rounded-xl text-white font-extrabold shadow-xl flex items-center justify-center gap-3 transition-all hover:scale-105 active:scale-95 ${timer ? 'bg-red-500 hover:bg-red-600 shadow-red-500/30' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-600/30'}`}>{timer ? <><Square fill="currentColor" size={20}/> STOP TIMER</> : <><Play fill="currentColor" size={20}/> START TIMER</>}</button>
    </div>
  );
}
