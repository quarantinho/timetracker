...
const GlobalStyles = () => (
  <style>{`
    ...
    /* STRICT CHART GRID ALIGNMENT - FIXED TO 4 COLUMNS */
    .chart-grid-row { 
      display: grid; 
    ...
    /* Strict Alignment for Sub-rows too */
    .chart-sub-row { 
      display: grid; 
      grid-template-columns: 40px 200px 1fr 80px; /* Same columns for alignment */
      align-items: center; 
      gap: 16px; 
      margin-bottom: 8px; 
      opacity: 0.8; 
        ...
        {projectsData.projects.map(p => {
          ...
          return (
            <div key={p.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all">
              ...
              {isExpanded && (<div className="p-4 bg-slate-50 border-t border-slate-100 animate-in slide-in-from-top-2"><div className="mb-4 flex gap-2"><input value={taskName} onChange={e=>setTaskName(e.target.value)} className="glass-input p-2 rounded-lg w-full text-sm font-medium outline-none" placeholder="New sub-task..." /><button onClick={() => addTask(p.id)} className="bg-slate-800 text-white px-4 rounded-lg font-bold text-sm"><Plus size={16}/></button></div><div className="space-y-2">{tasks.map(t => (<div key={t.id} className="flex justify-between items-center p-3 bg-white rounded-lg border border-slate-200 text-sm shadow-sm"><span className="text-slate-600 font-medium">{t.name}</span><button onClick={() => delTask(t.id)} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={14}/></button></div>))}</div></div>)}
            </div>
          );
        })}
      </div>
